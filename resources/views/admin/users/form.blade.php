...
@section('content')
    ...
    <div class="mb-8">
        ...
        <p class="mt-1 text-sm text-mist">
            {{ $editing ? 'Dados, sessões e histórico desta conta — cada bloco salva por conta própria.' : 'Defina identidade, foto e credenciais de acesso ao painel.' }}
        </p>
    </div>

    <div @class([
        'grid gap-8',
        'xl:grid-cols-[1.1fr_0.9fr]' => $editing,
        'max-w-3xl' => ! $editing,
    ])>
        <div class="space-y-6">
            @if($editing)
        ...
        @if($editing)
            <div class="space-y-6">
                <x-admin.session-list
                    ...
                    title="Sessões deste usuário"
                    description="Revogue logins ativos. Mostra IP, local, tipo, aplicativo e data/hora."
