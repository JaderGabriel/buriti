...
>
    ...
    <div class="pm-card__compact" aria-hidden="true">
        <span class="pm-card__compact-progress" data-progress-compact>
            @if($percent === null)
                Sem etapas
            @else
                {{ $percent }}%
        ...
        @if($open > 0)
            <span class="pm-flag pm-flag--open">{{ $open }} aberta{{ $open === 1 ? '' : 's' }}</span>
        @endif
    </div>

    <div class="pm-card__body" data-pm-card-body>
